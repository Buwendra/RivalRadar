...
> Phase 10a. Annual review tracker for RivalScan's sub-processors. Maps to SOC 2 CC9.2 (vendor and business-partner risk management) and GDPR Art. 28 (processor obligations). Companion to [SECURITY_OVERVIEW.md](SECURITY_OVERVIEW.md) § Sub-processors and [`/legal/sub-processors`](https://app.rivalscan.com/legal/sub-processors) (the public list customers see).
...
- **Off-cycle review (mandatory):**
  ...
  - Vendor announces a security incident affecting RivalScan customers' data.
...
| **Paddle** | Payments + merchant of record (subscriptions, tax, billing, invoicing) | Customer email, billing address, payment metadata (cardholder data handled entirely by Paddle — never reaches RivalScan) | UK / EU global | PCI DSS Level 1, SOC 2 Type II | 2026-05-07 | 2027-05-07 | Active |
...
5. **Have any incidents affected RivalScan during the year?** Cross-reference the vendor's status page history against `INCIDENT_RUNBOOK.md` post-incident logs.
...
| Vendor's SOC 2 report has a material exception | Document the exception in this register. Assess whether the exception affects RivalScan's controls. Consider compensating controls. |
...
These are the vendors whose downtime *immediately* breaks RivalScan:
