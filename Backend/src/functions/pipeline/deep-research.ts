...
export const handler = async (event: Event): Promise<Output> => {
  ...
  try {
    ...
    await putItem({
      ...
      // Persist the derived-state block (stage / funding / hiring / direction /
      // tech / pacing). It's generated by deepResearch() and consumed in-memory
      // by the enrichment step (threat + tags), but must also be written here so
      // the Comparator matrix and prior-vs-current delta detection can read it
