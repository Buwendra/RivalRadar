...
| **Split API stack into nested/sibling stacks** | API stack is at the 500-resource CloudFormation hard limit. Each new route burns 6 resources; we're forced to make tactical cuts (e.g. `POST /brand/setup` route is currently omitted) until this is fixed. | ~1 session refactor | Next time we need to add 1+ route and can't |
