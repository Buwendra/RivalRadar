...
2. **For org-level runaway** (cap doesn't apply because the cost wasn't user-attributable — system tasks don't bump per-user costs): manually disable the offending Lambda. AWS Console → Lambda → function → Configuration → Concurrency → reserve 0. **Blast-radius warning (domain functions):** API Lambdas are domain-grouped — reserving 0 on a function kills EVERY route it serves (e.g. `…-Api-Users` = 7 routes; the route→function map is `Backend/src/functions/route-manifest.ts`). The deliberate solo functions are safe surgical targets: `…-Api-AuthSignup` (signup kill switch — never touch `…-Api-AuthRefresh`, killing refresh locks out every signed-in user within ~1h), `…-Api-PaddleWebhook`, `…-Api-UserDelete`, `…-Api-ResearchRunsDetails`. All four paid-research trigger routes share `…-Api-ResearchTriggers` — reserving 0 there is the one-lever research kill (it also blocks onboarding, which calls `/users/onboard`). For a single route inside a shared function, the true per-route kill is deleting its gateway route (`aws apigatewayv2 delete-route`) — accepting CloudFormation drift until the next deploy — or a handler-level env flag like signup's `SIGNUP_ENABLED`.
...
*Last reviewed: 2026-05-07 (Phase 10a — added Annex A)*
