...
How to buy a custom domain and wire it to the RivalScan stack (Amplify frontend,
...
- **Brandable, no hyphens, easy to say aloud.** Target `rivalscan.com`; keep 2–3
...
- Map **root** (`rivalscan.com`) and **`www`**; pick one canonical (redirect the
  other to it).
...
- SES Console → **Verified identities → Create identity → Domain** → `rivalscan.com`.
- Enable **Easy DKIM** (adds 3 CNAMEs); add **SPF** and a **DMARC** record.
- Set a **custom MAIL FROM** subdomain (e.g. `mail.rivalscan.com`) for deliverability.
- Sending address e.g. `noreply@rivalscan.com` → point `FROM_EMAIL` at it.
...
- **Cleaner long-term:** add `api.rivalscan.com` via API Gateway *Custom domain
...
- `NEXT_PUBLIC_APP_URL`, `NEXT_PUBLIC_API_URL`, `NEXT_PUBLIC_APP_NAME` are **inlined
  ...
  ```
- `FRONTEND_URL` (backend/CDK, CORS) must **exactly match** `https://rivalscan.com`
...
- [ ] (Optional) `api.rivalscan.com` custom domain configured
