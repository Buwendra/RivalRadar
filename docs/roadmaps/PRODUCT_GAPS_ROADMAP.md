...
| **~~Split API stack into nested/sibling stacks~~ ✅ RESOLVED (Aug 2026)** | Superseded by the **domain-function consolidation**: ~84 per-route Lambdas → 20 domain-grouped functions with routeKey routers. Stack now synthesizes ~211/500 resources; a new route costs **1** resource (manifest row + router entry in `Backend/src/functions/route-manifest.ts`); `POST /brand/setup` is restored. See `docs/LAUNCH_ISSUES.md` Issue 6. | done | n/a |
