...
- 💤 Deferred — parked (e.g. Phase 11 — Go Live)
...
| # | Phase | Status | Theme | Effort | Depends on | Reversible? |
|---|---|---|---|---|---|---|
...
| 5 | Onboarding & top-of-funnel | ✅ (partial) | AI competitor discovery, mobile, tour | M | — | Yes |
...
| 9 | Security & compliance hardening | 🚧 ~80% | WAF, throttling, CloudTrail, npm audit, runbooks | M | — | Yes |
...
> **Shipped.** Confirmed by `slackIntegration` + `webhookIntegration` capability flags ([Backend/src/shared/types/capabilities.ts](../../Backend/src/shared/types/capabilities.ts)), the `IntegrationCredential` entity, and live `/integrations/*` routes. The notifier abstraction at `Backend/src/shared/services/notifiers/` is the runtime owner. Original implementation plan retained below for reference.
...
## ✅ Phase 5 — Onboarding & Top-of-Funnel (partial)

> Analytics events, resend-verification, and mobile responsive pass are live. AI competitor discovery + first-run tour shipped per `Backend/src/functions/api/onboarding/`. **Remaining**: deeper tour polish — pulled forward as a demo-wow candidate, not currently scheduled.
...
> ~80% shipped. Remaining items (per-route auth throttling, lawyer-finalised policy text, WAF attach via CloudFront fronting) all moved to **Phase 11 — Go Live**.
...
| **Per-route auth throttling (5 RPS on `/auth/*`)** | HTTP API v2 doesn't support per-route throttle properties (REST v1 only). Real fix is WAF rate-based rules with URI-path scope, which needs the CloudFront fronting above — so bundle with the WAF item. Default 100 RPS + Cognito lockout cover us until then. | bundled with WAF attach | Bundled with WAF attach |
| **Split API stack into nested/sibling stacks** | API stack is at the 500-resource CloudFormation hard limit. Each new route burns 6 resources; we're forced to make tactical cuts (e.g. `POST /brand/setup` route is currently omitted) until this is fixed. **Naive split tried and reverted (commit `7b7d305`)** — CFN can't migrate routes between stacks (every routeKey collided as `AlreadyExists`); proper fix needs `RETAIN` + `cdk import` per route, or two-step deploy with route deletion in between. | ~1–2 sessions with `cdk import` flow | Next time we need to add 1+ route and can't |
