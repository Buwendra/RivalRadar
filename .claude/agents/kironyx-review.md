...
**5. CORS header drift.** CORS exists in two places that must stay in step: the per-request `corsHeaders(event)` helper in `shared/middleware/handler.ts` (origin echo against the `ALLOWED_ORIGINS` env list; never `*` — `allowCredentials: true` forbids it) and the gateway `corsPreflight` in `lib/stacks/api.stack.ts` (built from the same `ALLOWED_ORIGINS`). Any new custom request header needs both allow-header lists. Also flag any handler that builds its own static CORS headers instead of calling `corsHeaders(event)` — `api/public/battlecard.ts` is the reference consumer for raw handlers.
...
**7. PDF route outside the `Pdf` function.** Both PDF routes live in the dedicated `Pdf` domain function (`pdfFonts: true` in `src/functions/route-manifest.ts`), which carries 1024 MB / 60s and the `commandHooks.afterBundling` hook copying PDFKit's `.afm` font metrics into the bundle. A PDF route assigned to any other function fails LAZILY: synth and cold start succeed, and the first actual render throws `Cannot find module ... data/Helvetica.afm`. If the diff adds a route whose handler renders with PDFKit, its manifest row must say `fn: 'Pdf'`.

**8. Manifest/router drift on new or moved routes.** A route exists only when BOTH the manifest row (`src/functions/route-manifest.ts`) and the owning router's dispatch entry (`src/functions/routers/<fn>.ts`) are present and agree on the exact routeKey string. `routers/router-manifest.test.ts` and `lib/stacks/api.stack.test.ts` enforce this in CI — if the diff touches routes, confirm both tests were updated in the same commit rather than loosened. Also check grouping intent: new routes join functions by PRIVILEGE and PACKAGING, not URL path (no elevated-IAM function gains a public route; nothing joins `Pdf` unless it renders PDFs; kill-switch solo functions — AuthSignup, AuthRefresh, PaddleWebhook, UserDelete, ResearchRunsDetails — stay solo). Routers must dispatch on exact `event.routeKey` only — flag any path parsing or prefix matching in a router.
