...
Tests are colocated next to source as `*.test.ts`. `vitest.config.ts` includes `test/**/*.test.ts`, `src/**/*.test.ts`, and `lib/**/*.test.ts` (the last one exists for the Api-stack template test) — if you add tests in a new place, confirm one of those patterns matches.
...
**CDK deploy** (required for `cdk deploy`): `CDK_DEFAULT_ACCOUNT`, `CDK_DEFAULT_REGION` (defaults to us-east-1), `FRONTEND_URL` (canonical origin — email links + single-origin CORS fallback), `ALLOWED_ORIGINS` (comma-separated full CORS allow-list, e.g. `https://kironyx.com,https://www.kironyx.com,https://main.d1zrq9gf129s9u.amplifyapp.com`; optional — defaults to `FRONTEND_URL` alone, and an empty value also falls back rather than deploying a dead allow-list. **Every origin the browser loads the app from must be in this list or CORS blocks it**), `FROM_EMAIL`, `PADDLE_PRICE_*`. Optional: `ALERT_EMAIL` (subscribes the alarms SNS topic AND enables the monthly AWS cost budget — the budget is skipped with a synth-visible warning when unset; **set it before a real deploy**), `AWS_BUDGET_MONTHLY_USD` (default $50). `bin/app.ts` validates `CDK_DEFAULT_ACCOUNT` and `FRONTEND_URL` at synth time. A populated `Backend/.env` can be sourced with `set -a && source .env && set +a`.
